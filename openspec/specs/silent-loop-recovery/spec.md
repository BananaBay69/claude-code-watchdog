# silent-loop-recovery Specification

## Purpose

Defines the dispatch contract that runs after `detect_silent_loop()` returns a positive detection. Selects between alert-only (default `disabled`), automated diagnostic capture (`snapshot-only`), and reserved future modes (`soft`, `aggressive`) based on `WATCHDOG_SILENT_LOOP_RECOVERY`. Snapshot-only mode writes a fixed file set under `$LOG_DIR/snapshots/` with FIFO retention, piggybacks on the existing alert dedup flag (one snapshot per state-entry, not per tick), and appends the snapshot path to the outgoing alert message so operators can triage without an interactive ssh round-trip. Manual `--snapshot` CLI flag exposes the same capture independent of detection state.
